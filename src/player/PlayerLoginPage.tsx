...
export default function PlayerLoginPage() {
  ...
  const { session, linked, linkLoading } = usePlayerLink();

  const [email, setEmail] = useState("");
  const [password, setPassword] = useState("");
  const [busy, setBusy] = useState(false);

  // If already authed, route them forward
  useEffect(() => {
    if (!session) return;
    if (linkLoading) return;
    nav("/player/dashboard");
  }, [session, linked, linkLoading, nav]);
  ...
  async function signup() {
    ...
    // Optional: you can navigate to claim, but if session is null it may bounce back.
    // nav("/player/claim", { replace: true });
  }
    ...
    <div className="container">
      <div className="card" style={{ maxWidth: 520, margin: "0 auto" }}>
        ...
        <div className="row" style={{ alignItems: "center", justifyContent: "space-between" }}>
          <div className="row">
            <button className="button buttonPrimary" onClick={login} disabled={busy}>
              Login
            </button>
