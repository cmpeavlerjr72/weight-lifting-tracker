...
export default function PlayerDashboardPage() {
  ...
  async function loadDashboard() {
    ...
    setLoading(true)
    try {
      const [tw, pr, sess, tr, comp] = await Promise.all([
        getPlayerTodayWorkout(playerId),
  ...
  // Compute total exercise completion for today's workout
  const todayTotalSets = todayWorkout?.exercises.reduce(
    (sum, ex) => sum + ex.setGroups.reduce((s, sg) => s + sg.sets, 0), 0
  ) ?? 0
  const todayCompletedSets = todayWorkout?.exercises.reduce(
    (sum, ex) => sum + ex.setGroups.reduce((s, sg) => s + sg.completedSets, 0), 0
  ) ?? 0
        ...
        {todayWorkout ? (
          <div className="card todayCard">
            <div className="row" style={{ alignItems: 'baseline', justifyContent: 'space-between' }}>
              <div>
                <div className="sectionTitle">Today's Workout</div>
                <div className="todayTitle">{todayWorkout.name}</div>
              </div>
              <div className="badge">{todayCompletedSets}/{todayTotalSets} sets done</div>
            </div>
            ...
            <div style={{ marginTop: 8 }}>
              <div className="progressBar">
                <div
                  ...
                  style={{ width: `${todayTotalSets > 0 ? Math.round((todayCompletedSets / todayTotalSets) * 100) : 0}%` }}
                />
              </div>
            </div>
            ...
            <div className="exerciseList" style={{ marginTop: 10 }}>
              {todayWorkout.exercises.map((ex, i) => {
                const totalSets = ex.setGroups.reduce((s, sg) => s + sg.sets, 0)
                const completedSets = ex.setGroups.reduce((s, sg) => s + sg.completedSets, 0)
                  ...
                  <div key={i} className="exerciseRow" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                      ...
                      <div className="exerciseProgress">
                        ...
                        <span className="small">{completedSets}/{totalSets}</span>
                      </div>
                    </div>
                    <div style={{ marginTop: 4 }}>
                      {ex.setGroups.map((sg, j) => (
                        <div key={j} className="exerciseMeta">
                          {sg.sets}x{sg.reps} @ {sg.targetWeight != null ? `${sg.targetWeight} lbs` : '? lbs'}
                          {sg.percentOfMax != null && (
                            <span style={{ opacity: 0.6 }}> ({sg.percentOfMax}%)</span>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <div className="card restCard">
            ...
            <div className="h2" style={{ marginBottom: 0 }}>No Workout Assigned Today</div>
